...
while (<FH>){
  ...
  foreach (split(/[,;]/, substr $_,(index $_,":")+1)) {
    $_ =~ s/^\s+//;
    my @ball = split (" ", $_);
    if ($ball[1] eq "red") { if ($ball[0] > $redMax) { $redMax = $ball[0]; } }
    if ($ball[1] eq "green") { if ($ball[0] > $greenMax) { $greenMax = $ball[0]; } }
    if ($ball[1] eq "blue") { if ($ball[0] > $blueMax) { $blueMax = $ball[0]; } }
  }
